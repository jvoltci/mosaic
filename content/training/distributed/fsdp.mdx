...
The fix is to **shard what was being replicated**. <Term name="zero">ZeRO</Term> (Microsoft, 2020) progressively chops the replicated state across data-parallel ranks: stage 1 shards the optimizer state, stage 2 adds gradients, stage 3 adds the parameters themselves. By stage 3, *no rank holds the full model* — each holds 1/N of every tensor and reconstructs full layers on the fly using <Term name="allgather">AllGather</Term>.

<Term name="fsdp">FSDP</Term> is PyTorch's implementation of ZeRO-3 (2022). FSDP2 (2024) is the per-parameter rewrite on top of <Term name="dtensor">DTensor</Term> — instead of wrapping whole modules, each parameter knows its sharding independently. It composes cleanly with tensor parallel and pipeline parallel, wraps faster, and gives finer-grained control over the AllGather/ReduceScatter overlap that determines whether your run hits 70% MFU or 30%.
...
| **ZeRO-2** | + gradients | full params + (1/N)(grads + optim) | <Term name="reducescatter">ReduceScatter</Term> grads |
...
For multi-node training, full FSDP across all ranks means cross-node AllGathers — expensive over IB / Ethernet. **<Term name="hsdp">HSDP</Term>** shards within a node (NVLink) and replicates across nodes (which use AllReduce instead of AllGather):
