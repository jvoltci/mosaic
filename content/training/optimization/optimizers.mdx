...
```python
def muon_step(W, m, lr, ns_steps=5):
    """Muon: momentum buffer + Newton-Schulz orthogonalization."""
    update = m + g          # momentum-buffered grad
    # Newton-Schulz iterates the matrix toward its "orthogonal" version
    X = update / update.norm()
    for _ in range(ns_steps):
        A = X @ X.T
        B = -2.0 * A + 1.5 * (A @ A)   # quintic NS iteration
        X = (1.5 * X) - (B @ X)
    W -= lr * X
    return W
```
