...
<RunInBrowser
  ...
  code={`def activation_gb(L, d, n_heads, n_kv_heads, ffn_mult, T, B, dtype_bytes=2, checkpoint='none'):
    """Rough peak activation memory for a Transformer training pass."""
    H = n_heads
    # Per layer, per token:
    attn_scores  = H * T * dtype_bytes        # the N×N attention matrix
    attn_io      = 4 * d * dtype_bytes        # q, k, v projections + output (rough)
    ffn          = ffn_mult * d * dtype_bytes # intermediate FFN
    per_layer_per_token = attn_scores + attn_io + ffn
    full_act_bytes = L * per_layer_per_token * T * B
    if checkpoint == 'full':
        # Save only at layer boundaries; recompute the rest. Approx √L reduction.
        full_act_bytes /= (L ** 0.5)
    elif checkpoint == 'selective':
        # Keep FFN, recompute attention. Approx 0.4x.
        full_act_bytes *= 0.4
    return full_act_bytes / 1024**3

configs = [
    ("Llama-7B  d=4096 H=32  L=32  ffn=11k T=8K  B=1  no-ckpt",  32, 4096, 32, 8, 11008, 8192, 1, 'none'),
    ("Llama-7B  same  selective ckpt                          ",  32, 4096, 32, 8, 11008, 8192, 1, 'selective'),
    ("Llama-7B  same  full ckpt                               ",  32, 4096, 32, 8, 11008, 8192, 1, 'full'),
    ("Llama-70B d=8192 H=64  L=80  ffn=28k T=8K  B=1  no-ckpt",   80, 8192, 64, 8, 28672, 8192, 1, 'none'),
]
print(f"{'config':<60}  {'activations':>14}")
print('-' * 80)
for label, *a in configs:
    print(f"{label:<60}  {activation_gb(*a):>11.2f} GB")
`}
/>
