...
PyTorch dominates research-volume in 2026; JAX dominates TPU work and a meaningful slice of frontier training (Anthropic, Google, parts of OpenAI). The JAX/XLA stack is the *other* world of AI compilation — same compiler concepts as PyTorch's <Term name="inductor">Inductor</Term> + <Term name="triton">Triton</Term>, but with a different IR (<Term name="stablehlo">StableHLO</Term>), a different graph compiler (<Term name="xla">XLA</Term>), a different kernel DSL (<Term name="pallas">Pallas</Term>), and a TPU backend that PyTorch simply doesn't have.
...
3. XLA receives the StableHLO module. Runs ~100 passes: <Term name="fusion">fusion</Term>, layout assignment, autotune, codegen.
