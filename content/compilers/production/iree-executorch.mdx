...
class MyModel(torch.nn.Module):
    def forward(self, x):
        return torch.relu(x @ self.linear.weight + self.linear.bias)
...
iree-compile --iree-hal-target-backends=cuda --iree-cuda-target=sm_90a my_model.mlirbc -o my_model.vmfb
