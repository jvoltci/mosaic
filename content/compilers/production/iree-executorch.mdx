...
IREE accepts <Term name="stablehlo">StableHLO</Term> (from JAX, TF, ONNX, or PyTorch via `torch-mlir`), lowers through MLIR <Term name="dialect">dialects</Term>, and emits a self-contained "VM module" (`.vmfb`) plus a hardware-specific binary blob.
...
3. Run high-level optimizations (op <Term name="fusion">fusion</Term>, layout selection).
4. <Term name="lowering">Lower</Term> to dispatch dialects (one dispatch per fused kernel).
