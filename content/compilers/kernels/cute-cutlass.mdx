...
<Term name="triton">Triton</Term> covers ~95% of kernel work, but the remaining 5% is where the most impactful kernels live: cuBLAS, FlashAttention-3 (its CUTLASS variant), the kernels NVIDIA's CUDA team ships, anything that needs to land on day one of a new architecture before Triton's backend catches up. **<Term name="cutlass">CUTLASS</Term> is how those kernels get written.** It's NVIDIA's open-source C++ template library for GEMM-class kernels, and the reference for what a fast tensor-core kernel looks like in production.

CUTLASS 3.x introduced <Term name="cute">**CuTe**</Term>, a layout sublanguage that's the unifying abstraction underneath everything. The mental shift: **everything is a layout.** A SMEM tile, a register fragment, a TMA descriptor, a swizzle pattern — all are CuTe `Layout` values composed by `tile`, `partition`, `coalesce`. Once you can read CuTe layouts, every modern NVIDIA GEMM is legible. Even if you write Triton, the layouts CuTe expresses (interleaved, swizzled, partitioned) are the same layouts Triton emits underneath — knowing CuTe makes Triton's IR dumps legible.

This lesson is the on-ramp: how CUTLASS is structured, what CuTe is, and when to reach for hand-written CUTLASS over Triton.
...
2. The **SMEM tile** of A after a TMA load (often <Term name="swizzle">**swizzled**</Term> to avoid bank conflicts).
...
Looks like a lot, but every line is a knob: types, layouts, tile shape, cluster shape, schedule (<Term name="tma">TMA</Term> + warp-specialized + ping-pong). The **CollectiveBuilder** picks the kernel implementation that matches all those constraints. There's no per-kernel hand-rolled C++ — it's all template instantiation against a few thousand lines of generic CUTLASS.
