...
**EAGLE-2** (Li et al., 2024) goes further: a small auto-regressive head that uses the target's hidden states to draft, achieving 80%+ acceptance on chat. Production result: 3–4× speedup, no separate draft model.
...
For phone deployment, EAGLE-2 is the right pick when memory is tight (one model, not two). Use a separate draft model when memory is comfortable and you can pick a high-quality off-the-shelf small model.
...
5. **Self-speculative variants (Medusa, EAGLE-2)** ship one model with extra heads instead of two models — better when memory is tight.
...
<Resources
  items={[
    ...
    { kind: 'paper', href: 'https://arxiv.org/abs/2406.16858', title: 'EAGLE-2: Faster Inference of Language Models with Dynamic Draft Trees', author: 'Li et al., 2024', note: 'Current frontier — 80%+ acceptance, 3–4× speedup. The right self-speculative method as of 2025.' },
