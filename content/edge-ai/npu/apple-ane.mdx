...
- The **Apple Neural Engine (ANE)** is a fixed-function NPU on every iPhone since A11 (2017) and every M-series Mac. 16-core in A17/M3+, ~38 TOPS at INT8. Lives next to the CPU and GPU on the same SoC die.
...
For this reason, Llama-style long-context (>4K) typically falls back to GPU on iPhone, even when the model "fits". This is **expected behavior** — don't chase it.
...
- Long-context attention (seq > 1024 token at d_model > 4096 → exceeds tile budget).
...
3. **The 16K-element cap** breaks long-context attention; expect LLMs >4K context to fall back to GPU. This is normal.
