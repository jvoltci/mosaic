...
```cpp
// Bad: each iteration touches two cache lines (8 bytes value, 56 bytes padding)
struct Particle {
    double x, y, z;       // 24 bytes
    char tag;              // 1 byte
    char padding[39];     // 39 bytes
    double mass;           // 8 bytes — straddles a cache line!
};

// Good: hot fields packed, cold fields after
struct Particle {
    double x, y, z, mass;  // 32 bytes — all in one cache line
    char tag;
    // ... cold fields ...
};
```
