...
C++ does the opposite. Every heap allocation has exactly one owner, and you encode that ownership in the *type* of the variable that holds it. When the owner goes out of scope, its <Term name="destructor">destructor</Term> runs and the resource is released. No garbage collector, no pauses, no surprises about *when* cleanup happens.
...
- Copy: one <Term name="atomic variable">atomic</Term> increment.
