...
**Prefix caching** is the trick that turns the 50,000th prefill into a hash lookup. Every <Term name="kv cache">KV cache</Term> block whose contents are determined by a prefix the server has already seen is served from a hash table. Same model, same numbers, **TTFT drops from 1.2 s to ~30 ms**. The 70B model didn't get faster; the workload got de-duplicated.
