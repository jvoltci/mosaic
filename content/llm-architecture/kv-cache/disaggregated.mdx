...
When a request hits Mooncake (Kimi's serving stack) or any 2025-era frontier production system, the architecture isn't "one GPU runs your prompt forward and streams tokens back." It's two pools. A **prefill pool** runs your prompt through every layer in one giant compute-heavy matmul. The resulting <Term name="kv cache">KV cache</Term> is then **transferred** over NVLink or RDMA to a **decode pool** that streams tokens to you. Two physically distinct GPU jobs, separated by a KV transport, glued by a router.
