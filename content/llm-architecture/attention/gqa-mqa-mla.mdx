...
- **MLA** (DeepSeek-V2/V3, 2024): K/V projected into a low-rank **latent** space, decompressed only when needed. Cache shrinks **another ~10×** vs GQA, with quality matching or exceeding MHA.
- **The tradeoff** is always cache size vs quality. As of April 2026, GQA is the floor and MLA is the frontier — Llama 4 (2025) adopted MLA-style compression in its largest variant.
...
For Llama-2 70B (MHA, $H = 64, d_h = 128, L = 80$) at $T = 8192$, BF16:

$$
2 \times 80 \times 64 \times 128 \times 8192 \times 2 \approx 21\text{ GB / sequence}
$$

This is why pre-2023 models maxed out around 4K context.
...
For DeepSeek-V3 (MLA + 128 Q heads, $d_c = 512$), per-token cache is one $c_t$ vector + a small RoPE-K vector — about **70 KB / sequence at 4K context** instead of GQA's ~3 GB. **20–30× smaller than GQA**, quality on par or better.
...
DeepSeek showed (V2, May 2024) and confirmed (V3, Dec 2024) that MLA matches or beats MHA on long-context benchmarks while keeping the cache an order of magnitude smaller. Llama 4 (April 2025) adopted similar latent-compression ideas in its long-context variants.

## Real-world adoption (April 2026)

| Model family | Attention | $H_q / H_{kv}$ |
| --- | --- | --- |
| Llama-1 / Llama-2 7B | MHA | 32 / 32 |
| Llama-2 70B | MHA | 64 / 64 |
...
| **Llama-4 (long-context)** | MLA-style | 2025 |

GQA is the universal floor. MLA is the open frontier.
