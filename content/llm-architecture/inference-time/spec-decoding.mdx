...
Per "speculation round" of $K$ tokens:
...
- Expected accepted prefix length: $\bar L = \sum_{i=1}^{K} \prod_{j \le i} \alpha$ where $\alpha$ is the acceptance probability (≈ same number of expected acceptances per step)

Speedup vs naive ≈ $\bar L \cdot c_{\text{target}} / (c_{\text{target}} + K \cdot c_{\text{draft}})$.

For a 70B target with a 1B draft and $\alpha \approx 0.7$, $K = 5$:
- $\bar L \approx 3.0$ tokens
...
<RunInBrowser
  ...
  code={`def speedup(K, alpha, c_draft_over_target=0.05):
    """
    K = max speculative depth
    alpha = per-token draft acceptance rate
    c_draft_over_target = compute cost of one draft step relative to one target step
    """
    # Expected accepted prefix length: 1 + α + α^2 + ... + α^K
    expected_accepted = sum(alpha ** i for i in range(1, K + 1))
    # Total cost per round: target forward + K draft forwards
    cost_round = 1 + K * c_draft_over_target
    return (1 + expected_accepted) / cost_round

print("Speedup as a function of draft acceptance rate α (K = 5, draft = 5% of target cost):")
for alpha in (0.3, 0.5, 0.6, 0.7, 0.8, 0.9):
    s = speedup(K=5, alpha=alpha)
    print(f"  α = {alpha:.1f}  ->  {s:.2f}x")

print("\\nSpeedup as a function of K (α = 0.7):")
