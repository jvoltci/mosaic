...
The bill comes due in odd places. A `transpose` followed by a `sum` along an axis reads memory at huge strides instead of sequentially. Each read is a fresh <Term name="cache miss">cache miss</Term>; on GPU each read fails to coalesce into a clean transaction. Same number of reads, same arithmetic — 5× to 20× slower wall-clock time.
...
Same bytes, same number of reads. The first pattern reuses one <Term name="cache line">cache line</Term> for 64/sizeof(elem) reads; the second pays for a fresh cache-line load every time.
